...
int main_till_6_5() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // draw
        // this part allows us to change the color depending on time
        float timeValue = glfwGetTime();
        float greenValue = (sin(timeValue) / 2.0f) + 0.2f;
        float redValue = (sin(timeValue) / 2.0f) + 0.5f;
        float blueValue = (sin(timeValue) / 2.0f) + 0.7f;
        
        // with this we can get the location of uniform in shader
